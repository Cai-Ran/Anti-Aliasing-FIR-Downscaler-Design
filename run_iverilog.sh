...
iverilog \
    -o "${tmp_dir}/run_iverilog.out" \
    -DCONFIG_FILE=\"./rtl_configs/${OLD_X}x${OLD_Y}-${NEW_X}x${NEW_Y}_define.v\" \
    -DIN_HEX_DIR=\"${IN_HEX_DIR}\" \
    -DOUT_HEX_DIR=\"${OUT_HEX_DIR}\" \
    -DPIC_NAME=\"${BASENAME}\" \
    -DIN_ROM_DIR=\"${IN_ROM_DIR}\" \
    -DGOLDEN_DIR=\"${GOLDEN_DIR}\" \
    ./sim/tb.v


echo "[VVP ${OLD_X}x${OLD_Y} ${NEW_X}x${NEW_Y} ${BASENAME}] Running simulation..."


stdbuf -oL vvp "${tmp_dir}/run_iverilog.out"
